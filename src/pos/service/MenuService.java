...
import pos.model.FriedChickenLeg;
import pos.model.FriedChickenBreast;
import pos.model.FriedChickenWhole;
import pos.model.Hotdog;
import pos.model.Longganisa;
import pos.model.Tocino;
import pos.model.Coke;
import pos.model.Sprite;
import pos.model.Royal;
import pos.model.Drink;
...
public class MenuService {
            ...
            try {
                ...
                if ("FriedChickenLeg".equals(cls)) mi = new FriedChickenLeg(id, name, price, r);
                    ...
                    if (r == null || r.getItems().isEmpty()) {
                        mi = new Drink(id, name, price);
                    } else {
                        mi = new MenuItem(id, name, price, r);
                    }
                }
                items.add(mi);
            } catch (Exception ex) {
    ...
    public void add(MenuItem m, String className) {
        ...
        String line = m.getId() + "|" + className + "|" + m.getName() + "|" +
                String.format("%.2f", m.getPrice()) + "|" + recipe;
    ...
    public void update(MenuItem m, String className) {
        ...
        for (String l : lines) {
            ...
            try {
                ...
                if (lid == m.getId()) {
                    String recipe = (m.getRecipe() == null ? "" : m.getRecipe().serialize());
                    out.add(m.getId() + "|" + className + "|" + m.getName() + "|" +
                            String.format("%.2f", m.getPrice()) + "|" + recipe);
                } else {
                    out.add(l);
                }
